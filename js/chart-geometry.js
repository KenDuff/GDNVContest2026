...
function buildArcChart(ctx) {
  ...
  const arcs = shown.map(o => {
    ...
    const col = rA === rB ? (E.regionColor[rA] || accent) : '#d8d3c8';
...
function buildBundleChart(ctx) {
  ...
  const curves = mp.map(o => {
    ...
    const baseCol = A.region === B.region ? (E.regionColor[A.region] || accent) : 'rgba(190,196,210,0.9)';
    const on = o.a === selNode || o.b === selNode;
    const touches = !selNode || on;
...
function buildMatrixChart(ctx) {
  ...
  // Data-mark colour follows the vote channel, matching the record discs:
  // jury = teal, public = red (all-time / orange keeps its own accent).
  const voteCol = openKey === 'crimson' ? '#20a892' : openKey === 'teal' ? '#c8324c' : accent;
  ...
  const cells = raw.slice(0, keepN).map(c => {
    ...
    return {
      ...
      fill: (anySel && active) ? '#f6efda' : voteCol,
      pe: (anySel && !active) ? 'none' : 'auto',
      enter: chartHov(Object.assign({
        kind: 'edge', accent: voteCol, title: (E.names[c.A] || c.A) + '  \u2192  ' + (E.names[c.B] || c.B), sub: 'Avg points given \u00b7 ' + chanLbl,
...
function buildScatterChart(ctx) {
  ...
  const dots = shownPts.map(p => {
    ...
    // jury = teal, public = red — consistent with the record discs & legend.
    const col = juryFav ? '#20a892' : '#c8324c';
    const touches = !selNode || p.a === selNode || p.b === selNode;
