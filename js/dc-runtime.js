...
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    // (self-fetch-and-reapply-raw-template hot-reload hook removed: it exists to
    // pick up live edits in Claude's own artifact editor, but for a static
    // standalone export it only serves to refetch this exact file and splat the
    // *pre-substitution* placeholder markup back over the already-rendered page
    // whenever fetch(location.href) succeeds, e.g. when served over http(s)
    // rather than file://.)
    const dc = doc.querySelector("x-dc");
