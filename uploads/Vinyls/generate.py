...
def _load_regions():
    # gold.svg (Bloc-to-Bloc Ties) labels its 9 wedges with region names, not
    # country codes -- e.g. "Nordic", "Balkans" -- so it needs its own
    # matching set, not COUNTRIES2/ISO3TO2 which are for the other 6 records.
    euro_data_path = os.path.join(HERE, '..', '..', 'js', 'euro-data.js')
    with open(euro_data_path) as f:
        text = f.read()
    m = re.search(r'"regionColor":(\{[^}]*\})', text)
...
def _source_path(key):
    # silver/gold are never regenerated (no slider, no stripped background) --
    # read the actual served file directly rather than keeping a duplicate
    # copy in source_svgs/ just for this function to open.
    if key in ('silver', 'gold'):
        return os.path.join(HERE, f'{key}.svg')
    return os.path.join(SRC_SVG_DIR, f'{key}.svg')

def parse_source_svg(key):
    """Node centres + every original edge (with real a/b country codes,
    or region names for gold -- see REGIONS above)."""
    region_mode = (key == 'gold')
    with open(_source_path(key)) as f:
        text = f.read()

    # Some records' edges are coloured with a per-edge <linearGradient> in
    # the original artwork rather than a flat stroke colour. That's fine
    # inside that SVG file, but the interactive overlay is a *different*,
    # separately-rendered SVG with no access to those <defs> -- a
    # stroke="url(#ei3)" reference there resolves to nothing and the edge
    # renders invisible even at full opacity. Resolve every gradient to its
    # first stop's colour up front so nothing downstream ever sees a URL.
    gradRe = re.compile(r'<linearGradient id="([^"]+)"[^>]*>\s*<stop offset="0%" stop-color="([^"]+)"')
    gradMap = {gid: col for gid, col in gradRe.findall(text)}
    def resolve_color(raw):
        if raw.startswith('url(#') and raw.endswith(')'):
            return gradMap.get(raw[5:-1], raw)
        return raw
    ...
    for m in eRe.finditer(text):
        ...
        tag = m.group(0)
        resolved_col = resolve_color(m.group(7))
        if resolved_col != m.group(7):
            tag = tag.replace(f'stroke="{m.group(7)}"', f'stroke="{resolved_col}"', 1)
        raw_edges.append((x1,y1,x2,y2,resolved_col,float(m.group(8)),float(m.group(9)), m.group(10) is not None, tag))
    ...
    for l in labels:
        raw = l['raw'].strip()
        if region_mode:
            code = raw if raw in REGIONS else None
        else:
            code = ISO3TO2.get(raw, raw if raw in COUNTRIES2 else None)
            if code and code not in COUNTRIES2:
                code = None
        if not code:
            continue
        bi, bd = -1, 1e9
